...
colorPicker.directive('colorPicker', ['$document', '$compile', '$timeout', 'ColorHelper', function ($document, $compile, $timeout, ColorHelper) {
        return {
            restrict: 'A',
            ...
            link: function (scope, element, attr) {
                ...
                function open(event) {                    
                    ...
                    $timeout(function () {
                        var satElem = template[0].getElementsByClassName("saturation-lightness")[0];
                        var hueElem = template[0].getElementsByClassName("hue")[0];
                        var alphaElem = template[0].getElementsByClassName("alpha")[0];
                        scope.sAndLMax = {x: satElem ? satElem.offsetWidth : 0, y: satElem ? satElem.offsetHeight : 0};
                        scope.hueMax = {x: hueElem ? hueElem.offsetWidth : 0};
                        scope.alphaMax = {x: alphaElem ? alphaElem.offsetWidth : 0};
                        scope.update();
                        setDialogPosition();
